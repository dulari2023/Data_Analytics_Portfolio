...
FROM `case-studies-405816.Top_100_billionaires.Billionaires_info` 
ORDER BY NET_WORTH__BILLIONS_ DESC
LIMIT 5;
...
SELECT AVG(NET_WORTH__BILLIONS_) AS average_net_worth
FROM `case-studies-405816.Top_100_billionaires.Billionaires_info` 


-- 8.   How many billionaires in the dataset are above the average of the net worth in the dataset?

SELECT COUNT(*) AS number_of_billionaires_above_average
FROM `case-studies-405816.Top_100_billionaires.Billionaires_info` 
WHERE NET_WORTH__BILLIONS_ > (SELECT AVG(NET_WORTH__BILLIONS_) FROM `case-studies-405816.Top_100_billionaires.Billionaires_info`)


-- 9.   How many billionaires in the dataset are over the age of 60?

SELECT COUNT(*) AS number_of_billionaires_above_60
FROM `case-studies-405816.Top_100_billionaires.Billionaires_info` 
WHERE AGE > 60;
...
--Youngest
SELECT MIN(AGE) AS younguest_age,
FROM `case-studies-405816.Top_100_billionaires.Billionaires_info` 
